...
## Learning Goal
Protect APIs using JSON Web Tokens.

## Simple Explanation
JWT authentication proves who the caller is using a signed token.
...
## Real-world Example
A login endpoint returns a token that the frontend sends in Authorization header.
...
flowchart LR
    ...
    API --> Lesson[Authentication with JWT]
