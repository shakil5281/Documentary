# Employee Management API - Leave Module

## Learning Goal
Design the Leave module of a real-world Employee Management API from features to endpoints, database ideas, validation, service flow, and diagrams.

## Simple Explanation
The Leave module is one business area of the Employee Management API. A strong API design starts with user needs, then turns them into endpoints, tables, request/response DTOs, validation rules, and service logic.
...
## Database Table Idea
- `LeaveRequests`: stores leaverequests data.
- `LeaveBalances`: stores leavebalances data.
...
## Real-world Example
In a company HR system, the Leave module supports daily HR operations while keeping employee data consistent and auditable.
...
## Practice Task
Create one extra endpoint for the Leave module. Define the URL, HTTP method, request body, response body, validation rules, and service method name.

## Mermaid Diagram
```mermaid
flowchart TD
    UI[Frontend or Mobile App] --> Controller[Leave Controller]
    Controller --> Validator[Request Validation]
    Validator --> Service[Leave Service]
    Service --> Repository[Leave Repository]
    Repository --> DB[(LeaveRequests, LeaveBalances)]
